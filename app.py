...
with col1:
    with open("Sample.xlsx", "rb") as sample_file:
        st.download_button("📥 Download Input Template (Sample.xlsx)", sample_file, file_name="Sample.xlsx", key="download_sample_1")
with col2:
    with open("Summary.xlsx", "rb") as summary_file_download:
        st.download_button("📥 Download Summary Template (Summary.xlsx)", summary_file_download, file_name="Summary.xlsx", key="download_summary_1")
...
if use_manual_summary:
    manual_summary["Contract Identification No"] = contract_no_input
...
if data_file:
    ...
    def plot_chainage_subplot(entry, ax):
        ...
        try:
            ...
            chainage_str = f"{ch_int // 1000}+{ch_int % 1000:03d}"
        except:
